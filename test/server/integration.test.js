...
const { PIECE_TYPES, SPAWN_X, SPAWN_Y } = require(
  "../../src/server/constants.cjs",
);
    ...
    socket.on("startGame", ({ room }) => {
      ...
      game.players.forEach((p) => {
        ...
        io.to(p.id).emit("gameStarted", {
          pieceIndex: ti,
          piece: { type: t, spawnX: SPAWN_X[t], spawnY: SPAWN_Y },
        });
      });
    });
...
describe("Server Integration (socket.io)", () => {
  ...
  test("requestNextPiece → newPiece reçu", async () => {
    ...
    await Promise.all([
      event(c1, "gameStarted"),
      Promise.resolve(c1.emit("startGame", { room: "r11" })),
    ]);
