...
import Game, { buildDropTrail } from "../../src/client/components/Game";
...
describe("Game Component", () => {
  ...
  describe("Rendering", () => {
    ...
    it("renders GameOver screen when game is over", () => {
      const state = makeState({
        game: { room: "r1", started: true, over: true, players: [] },
      });
  ...
  describe("Game Loop (onTick)", () => {
    ...
    it("locks piece after LOCK_DELAY on blocked tick (last-frame rule)", async () => {
      ...
      expect(emitUpdateSpectrum).toHaveBeenCalledWith(
        "room1",
        expect.any(Array),
      );
      jest.useRealTimers();
    ...
    it("does not tick when piece or board is missing", () => {
      ...
      act(() => {
        if (capturedOnTick) capturedOnTick();
      });
...
describe("buildDropTrail", () => {
  it("spans each occupied column from departure to landing", () => {
    // Pièce J : la colonne 0 porte deux cases (r0 et r1), les autres une seule.
    const piece = { shape: [[1, 0, 0], [1, 1, 1]], x: 2, y: 3, type: "J" };
    const trail = buildDropTrail(piece, 15, 7);

    expect(trail).toEqual({
      seq: 7,
      type: "J",
      cols: [
        { x: 2, from: 3, to: 16 }, // haut en r0, bas en r1
        { x: 3, from: 4, to: 16 },
        { x: 4, from: 4, to: 16 },
      ],
    });
  ...
  it("skips columns the shape does not occupy and clamps above the ceiling", () => {
    // Pièce T au spawn : la colonne du milieu part de la rangée 0, les autres
    // de la rangée 1 — aucune ne peut commencer au-dessus du plafond.
    const piece = { shape: [[0, 1, 0], [1, 1, 1]], x: 4, y: -1, type: "T" };
    const trail = buildDropTrail(piece, 18, 1);

    expect(trail.cols.map((c) => c.x)).toEqual([4, 5, 6]);
    expect(trail.cols.every((c) => c.from >= 0)).toBe(true);
  });

  it("produces no band for an empty shape column", () => {
    const piece = { shape: [[1, 0, 1]], x: 0, y: 0, type: "I" };
    expect(buildDropTrail(piece, 5, 1).cols.map((c) => c.x)).toEqual([0, 2]);
  });
});
