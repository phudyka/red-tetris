...
describe("socket.js — Emitters", () => {
  ...
  it("emitJoinGame", () => {
    ...
    expect(mockEmitFn).toHaveBeenCalledWith("joinGame", {
      room: "room1",
      playerName: "Alice",
    });
  });
  ...
  it("emitRequestNextPiece", () => {
    ...
    expect(mockEmitFn).toHaveBeenCalledWith("requestNextPiece", {
      room: "room1",
    });
  });
  ...
  it("emitUpdateSpectrum", () => {
    ...
    expect(mockEmitFn).toHaveBeenCalledWith("updateSpectrum", {
      room: "room1",
      spectrum: [1, 2, 3],
    });
  });
...
describe("socket.js — Event Listeners → Redux dispatch", () => {
  ...
  it("gameJoined → GAME_JOINED + SET_PLAYER + SET_OPPONENTS (filtre soi-même)", () => {
    ...
    const setOpp = dispatch.mock.calls.find((c) =>
      c[0].type === "SET_OPPONENTS"
    );
  ...
  it("playerJoined → PLAYER_JOINED + ADD_OPPONENT", () => {
    ...
    const addOpp = dispatch.mock.calls.find((c) =>
      c[0].type === "ADD_OPPONENT"
    );
  ...
  it("gameStarted → GAME_STARTED + NEW_PIECE", () => {
    mockListeners["gameStarted"]({
      pieceIndex: 2,
      piece: { type: "T", spawnX: 3, spawnY: 0 },
    });
    const types = dispatch.mock.calls.map((c) => c[0].type);
  ...
  it("newPiece → NEW_PIECE", () => {
    mockListeners["newPiece"]({
      pieceIndex: 1,
      piece: { type: "I", spawnX: 3, spawnY: 0 },
    });
    const types = dispatch.mock.calls.map((c) => c[0].type);
    expect(types).toContain("NEW_PIECE");
  });
  ...
  it("opponentDead → OPPONENT_DIED avec le nom du joueur", () => {
    ...
    const action = dispatch.mock.calls.find((c) =>
      c[0].type === "OPPONENT_DIED"
    );
  ...
  it("disconnect → CONNECTION false, connect → CONNECTION true", () => {
    mockListeners["disconnect"]();
    expect(dispatch).toHaveBeenCalledWith({
      type: "CONNECTION",
      payload: { connected: false },
    });

    mockListeners["connect"]();
    expect(dispatch).toHaveBeenCalledWith({
      type: "CONNECTION",
  ...
  it("ne rejoint pas deux fois à la première connexion", () => {
    sock.emitJoinGame("r1", "Alice");
  ...
  it("rejoint la room après une coupure : le serveur a oublié la socket", () => {
    sock.emitJoinGame("r1", "Alice");
    mockListeners["gameJoined"]({
      room: "r1",
      playerName: "Alice",
      isHost: true,
      players: [],
  ...
  it("error → GAME_ERROR (affiché par App)", () => {
    mockListeners["error"]({ message: "test-err" });
    expect(dispatch).toHaveBeenCalledWith({
      type: "GAME_ERROR",
      payload: { message: "test-err" },
    });
  });
});
